...
def get_parser():
    ...
    parser.add_argument('--root-data', type=str, help='sampled video')
    parser.add_argument('--root-all-data', type=str, help='full video')
    parser.add_argument('--list-path', type=str)
    parser.add_argument('--epoch', type=int)
    parser.add_argument('--start-epoch', type=int, default=0)
    parser.add_argument('--sample-size', type=int, default=10)
    parser.add_argument('--gpu', type=str, default='0', help='specify gpu(s)')
    ...
    parser.add_argument('--show-step', type=int, default=5)
    parser.add_argument('--flow-scale', type=int, default=1)
    parser.add_argument('--flow-model', type=str, default='FlowNet2C')
...
def main():
    ...
    model.eval()
    flow_infer = Inference_flow(args, resume=args.restore)
    ...
    if args.restore != None:
        ...
        checkpoint = {k: v for k, v in checkpoint['seg'].items() if k in state}
    ...
    spec_obj_ind = [1,2,3] if not test_mode else None
    ...
    testloader = data.DataLoader(
        Valset(root_data=args.root_data, root_all_data=args.root_all_data, json_meta_list=args.list_path,
                   sample_size=args.sample_size, fix_size=False, half_size=True,
    ...
    for i_iter, batch in enumerate(testloader):
        ...
        for start in range(img.shape[1]):
            ...
            for i in tqdm(range(start+1, img.shape[1])):
                flow = flow_infer.infer(ori_img[i], ori_img[i-1], scale=args.flow_scale)
                ...
                for idx in range(prev_lab.shape[0]):
                    if (prev_lab[idx]>0.5).sum()==0:
                        output[idx,0] *= 0
        ...
        ### save & show result ###
        step = 5 if test_mode else args.show_step
        for i,lab in enumerate(save_lab):
            ### the only vid that has special interval ###
            if vid_name=='8aa47fac99':
                step = 3 if i < 80 else 5
            if i % step == 0:
