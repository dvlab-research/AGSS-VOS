...
from networks_flow.channelnorm_package.modules.channelnorm import ChannelNorm

from networks_flow import FlowNetC
from networks_flow import FlowNetS
from networks_flow import FlowNetSD
from networks_flow import FlowNetFusion

from networks_flow.submodules import *
...
class FlowNet2C(FlowNetC.FlowNetC):
    ...
    def forward(self, inputs):
        ...
        # logger.info(('x1', x1.shape))

        # FlownetC top input stream
        out_conv1a = self.conv1(x1)
        # logger.info(('out_conv1a', out_conv1a.shape))
        out_conv2a = self.conv2(out_conv1a)
        # logger.info(('out_conv2a', out_conv2a.shape))
        out_conv3a = self.conv3(out_conv2a)
        # logger.info(('out_conv3a', out_conv3a.shape))
        ...
        # logger.info(('out_corr', out_corr.shape))
        # Redirect top input stream and concatenate
        out_conv_redir = self.conv_redir(out_conv3a)

        # logger.info(('out_conv_redir', out_conv_redir.shape))
        in_conv3_1 = torch.cat((out_conv_redir, out_corr), 1)

        # Merged conv layers
        out_conv3_1 = self.conv3_1(in_conv3_1)

        # logger.info(('out_conv3_1', out_conv3_1.shape))
        out_conv4 = self.conv4_1(self.conv4(out_conv3_1))

        # logger.info(('out_conv4', out_conv4.shape))
        out_conv5 = self.conv5_1(self.conv5(out_conv4))
        # logger.info(('out_conv5', out_conv5.shape))
        out_conv6 = self.conv6_1(self.conv6(out_conv5))
        # logger.info(('out_conv6', out_conv6.shape))
        ...
        # logger.info(('out_deconv2', out_deconv2.shape))

        flow2 = self.predict_flow2(concat2)

        # assert 1<0

        if self.training and 1<0:
